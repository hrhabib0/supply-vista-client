...
const MyProfile = () => {
    ...
    // dynamic title
    useEffect(() => {
        document.title = "My Profile | SupplyVista";
    }, [])
    return (
        <div className='flex justify-center items-center my-8 mx-5 sm:mx-3 xl:mx-0'>
            <div className=''>
                <div className='text-center'>
                    ...
                    <h1 className='text-2xl font-bold border-2 border-blue-500 rounded-md my-4 p-2'>Name: {user.displayName}</h1>
                    <p className='border-2 border-blue-500 rounded-md my-2 p-2'>Email: {user.email}</p>
                    <p className='text-sm underline text-blue-400 cursor-pointer border-2 border-blue-500 rounded-md p-2'>User's Photo: <a href={user.photoURL}>{user.photoURL}</a></p>
                </div>
                <div className='my-6 bg-gray-200 p-4 rounded-2xl'>
                    ...
                    <form onSubmit={handleUpdateUserInfo} className='fieldset'>
                        ...
                        <button type='submit' className='btn btn-soft btn-primary'>Save</button>
                    </form>
                </div>
            </div>
        </div>
